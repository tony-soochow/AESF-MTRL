...
from utils import cfg_read, np_softmax
...
class Learner():
    def __init__(self, 
        ...
        self.logger = Logger(writer=self.writer, cfg_path=cfg_path, server=self.server, num_tasks=self.num_tasks)
        self.logger.start() # start thread's activity


        self.weights = [1/self.num_tasks for i in range(self.num_tasks)]
    ...
    def set_cfg_parameters(self, save_period, write_mode):
        ...
        self.datetime = str(datetime.now())[:-16]
        ...
        self.log_file = './log/DM_Distributed_GAN_log_{}.txt'.format(
            self.datetime
        )
        
        self.save_model_path = 'saved_models/DM_Distributed_GAN/{}/'.format(
            self.datetime
        )
        ...
        if self.write_mode:
            self.writer = SummaryWriter(
                './log/log_DM_Distributed_GAN/{}'.format(self.datetime)
            )
    ...
    def update_SAC(self, 
        ...
        with torch.no_grad():
            sampled_next_actions, next_log_probs, _, _ = self.actor.get_action_log_prob_log_std(
                mtobss=next_mtobss
            )
        ...
        sampled_actions, log_probs, log_stds, task_output = self.actor.get_action_log_prob_log_std(
            mtobss=mtobss,
        )

        task_label = mtobss[:, -self.num_tasks:]
        task_label = torch.argmax(task_label,-1)
        adv_loss = nn.CrossEntropyLoss()(task_output, task_label)
    ...
    def update(self):
        ...
        mtobss, actions, rewards, next_mtobss, dones = self.memory.sample(self.weights)
    ...
    def run(self):
        ...
        for update_iteration in itertools.count():            
            ...
            if update_iteration % 100000 == 0 and update_iteration != 0:
                self.weights = np_softmax(np.array([self.logger.reward_tar[i] - np.mean(self.logger.reward_lst[i]) if self.logger.reward_tar[i] - np.mean(self.logger.reward_lst[i]) > 0 else 0 for i in range(self.num_tasks)]))
                self.weights = np.clip(self.weights,0.3,0.7)

            critic_loss, actor_loss, entropy = self.update()

            # increase lambda
            self.actor.model.alpha += 1/20000
            self.actor.model.alpha = self.actor.model.alpha if self.actor.model.alpha < 1.0 else 1.0
