...
from utils_actor import BootstrappedNet
...
class Actor(nn.Module):
    ...
    def build_model(self, actor_cfg):
        self.model = BootstrappedNet(actor_cfg, self.num_tasks)

    def forward(self, mtobss : torch.Tensor):
        idx = mtobss[:,-self.num_tasks:]
        idx = torch.argmax(idx, -1)
        x, task_output = self.model.forward(mtobss, idx)
        mu, log_std = x[:, :self.action_dim], torch.clamp(x[:, self.action_dim:], -20, 2)
        std = torch.exp(log_std)
        return mu, std, task_output

    def get_action_log_prob_log_std(self, mtobss : torch.Tensor):
        mu, std, task_output = self.forward(mtobss=mtobss) # mu = (batch, num_action), std = (batch, num_action)
        ...
        return action, log_prob, log_std, task_output

    def get_action(self, mtobss : torch.Tensor, stochastic=True):
        if not stochastic:
            self.eval()

        mu, std, _ = self.forward(mtobss=mtobss) # mu = (batch, num_action), std = (batch, num_action)
...
class Actor_multihead(BootstrappedNet):
    ...
    def get_action_log_prob_log_std( self, mtobss : torch.Tensor):
        idx = mtobss[:, -self.num_tasks:]
        idx = torch.argmax(idx, -1)
        mean, std = self.forward(mtobss, idx)
    ...
    def cal_loss(self,
        ...
        loss = torch.mean(loss)
        if adv_loss:
            print("classify loss: ", adv_loss, "ori loss", loss)
            loss += torch.mean(adv_loss)
